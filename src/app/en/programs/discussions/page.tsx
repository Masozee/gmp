...
const DiscussionsPage = () => {
  const programTitle = "Public Discussions";
  const programHeroDescription = "Open discussion forum addressing current political issues relevant to young people.";
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-16 max-w-7xl">
        <h2 className="text-3xl font-bold mb-12 text-center text-primary" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 800 }}>Activity Documentation</h2>
        ...
        {!isLoading && !error && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {pastEvents.length === 0 ? (
              <p className="col-span-full text-center text-gray-500">No activity documentation yet.</p>
            ) : (
              pastEvents.map((event, index) => (
                <Link 
                  key={index} 
                  href={`/en/programs/discussions/${event.slug}`}

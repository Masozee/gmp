...
export default function EditProgramPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchProgram = async () => {
    try {
      const response = await fetch(`/api/admin/program/${programId}`);
      if (response.ok) {
        ...
        if (program.content) {
          try {
            const parsedSections = JSON.parse(program.content);
        ...
        if (program.contentEn) {
          try {
            const parsedSectionsEn = JSON.parse(program.contentEn);
            setSectionsEn(parsedSectionsEn);
          } catch (e) {
            console.error('Error parsing contentEn:', e);
          }
        }
      }
    } catch (error) {
          ...
          <TabsContent value="basic">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Edit informasi dasar tentang program
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
